...
<!-- 引入layui库 -->
<script defer src="/assets/qblog/layui/layui.js"></script>
<style type="text/css">
	/** 图标字体 **/
@font-face {
...
<span>/</span><a href="<?php  echo "/b".$v['tid']."/list-".$v['tid']?>.html" style="color: #7d8c8e;"><?php  echo $class[$v['tid']]?></a><span>/</span>正文</div>
<?php
...
?>
<audio id="audioPlayer" controls preload style="max-width: 50%;opacity: .4;float: right;height: 24px;margin-top: -6px;" title="收听本文" src="https://dict.youdao.com/dictvoice?le=zh&audio=<?php echo $r[0]; ?>">
  您的浏览器不支持 audio 元素。
</audio>
<span id="translate" style="border: 0px;cursor: pointer;float: right;color: #7d8c8e;font-size: 12px;margin-top: -5px;"></span>
    ...
    <script>
        ...
        const audioFiles = [
            <?php
             foreach ($r as $value) {
             echo "'https://dict.youdao.com/dictvoice?le=zh&audio=$value',";
             }?>
        ];
        let currentTrack = 0;
...
<div class="s_e"><input name="purl" tabindex="3" placeholder="网址(选填)" id="purl" type="url" class="input_narrow" value="<?php echo @$_COOKIE['purl'];?>" maxlength="50" /><input type="hidden" id="tid" name="tid" value="<?php echo $tid=$v['tid'];?>"></div>
</div>
	 <?php  if($safecode==1){?>
