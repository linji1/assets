...
function mbStrSplit ($string, $len = 1) {
    ...
    while ($strlen > 0) {
        ...
        $start += $len;
        $strlen = mb_strlen($string, "utf8") - $start;
    }
    return $array;
}
...
// 假设 $v 是一个关联数组，包含 'title' 和 'content' 键
...
// 过滤字符串，保留中文字符、英文字母、数字和特定标点
$str = match_chinese($str);

// 计算字符数，忽略空白字符
$zishu = mb_strlen(preg_replace('/\s/', '', html_entity_decode($str)), 'UTF-8');

// 按指定长度分割字符串
$r = mbStrSplit($str, 450);

// 音频URL前缀
$qian = "https://dict.youdao.com/dictvoice?le=zh&audio=";
?>
<?php if ($zishu <= 999999999): ?>
<center><audio id="audioPlayer" controls preload style="max-width: 50%;opacity: .4;float: right;height: 24px;margin-top: -6px;" title="收听本文" src="<?php echo $qian.$r[0]; ?>">
  您的浏览器不支持 audio 元素。
</audio></center>
    <script>
