...
<?php include "head.php";?>
  <div id="content">
    ...
    <?php if(empty($list)){ echo '<div class="post_list">没有找到相关记录...</div>';}else{foreach($list as $v){ ?>	 
      ...
      <div class="excerpt">
	  <?php if($v['pic']<>""){ ?><div class="thumbnail"><a href="<?php echo "/b".$v['tid']."/".vurl($v['id']); ?>"><img src="<?php echo $v['pic'];?>" /></a></div><?php } ?>
      ...
      <p class="navPost"><i class="iconfont icon-view"></i> 浏览(<?php echo $v['pv']; ?>) <a href="<?php echo "/b".$v['tid']."/".vurl($v['id']);?>#comments"><i class="iconfont icon-com"></i> 评论(<?php echo  $v['num']; ?>)</a> 
