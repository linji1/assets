...
<?php include "head.php";?>
  ...
  <div id="main1"<?php if($widget=="0") echo ' class="w100"';?>>
     <div id="comments">
		...
		<span class="reply"><a href="<?php echo '/b'.$v['ctid'].'/'.vurl($v['cid']).'#Com-'.$v['id'];?>">[查看]</a> <?php pl_admin( $v['id'], $v['cid'], $v['isn'], $v['pmail']);?></span>
		</div>
		<p><?php if($v['isn']==1 && $admin===0){echo '评论审核中...'; } else { echo nl2br($v['pcontent']);}?></p>
		<?php if($v['rcontent']<>""){?><p class="re">&nbsp;&nbsp;<span style="color:#C00"><?php echo $set['webuser']; ?>回复</span>：<span><?php echo $v['rcontent']; ?></span></p><?php }?>
		</div>
		</li>
       <?php } ?>
		</ol>
