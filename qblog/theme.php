...
function comment($n){
	...
	foreach($rs as $v){	
	  ...
	  $str .= '<li><a href="'. vurl($v['cid']) .'#Com-'.$v['id'].'">'.$v['pname'].' : '.$pcontent.'</a>';
...
function getprenext($id, $pn) {
    ...
    if ($pn == 'pre') {
      $sql = "SELECT id, tid, title FROM `log` WHERE atime < (SELECT atime FROM `log` WHERE id = :id) $where ORDER BY atime DESC LIMIT 1";
      $rs = $db->getdata($sql, ['id' => $id]);
      $t = '上一篇文章';
  } else {
      $sql = "SELECT id, tid, title FROM `log` WHERE atime > (SELECT atime FROM `log` WHERE id = :id) $where ORDER BY atime ASC LIMIT 1";
      $rs = $db->getdata($sql, ['id' => $id]);
      $t = '下一篇文章';
  }
    
    if (empty($rs)) {
        return '<div style="height: 3.2em;overflow: hidden;">无更多文章</div>';
    } else {
        return '<a class="' . $pn . '" href="' .'/b'.$rs[0]['tid'].'/'. vurl($rs[0]['id']) . '"><div style="height: 3.2em;overflow: hidden;">' . ($rs[0]['title'] == '' ? $t : $rs[0]['title']) . '</div></a>';
    }
}
