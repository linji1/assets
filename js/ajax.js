...
function plsave(id,pid,x,pmail){	
	...
	$.post("https://linji.cn/app/class/ajax.php?act=plsave&id=" + pid + "&cid=" + id, {
		rcontent: rlog
	}, function(data) {
        capl();
		if (data.result == '200') {
			...
			$.post("https://linji.cn/app/class/ajax.php?act=pltz&d=plsave&id="+pid,{rcontent:rlog,pmail:pmail,r:window.location.href});			 
		} else {
...
function ckpass(id){	
	var ps= $("#password").val();
	if (ps!=''){
	$.post("https://linji.cn/app/class/ajax.php?act=ckpass&id="+id, {ps:ps}, function(data) {if(data.result=='200'){ $("#article .text").html(data.message)}else{myalert(data.message);}},'json');}else{
	$("#password").focus();
	}	
}
...
$(document).ready(function() {
    ...
    function highlightActiveLink(links) {
        links.each(function() {
            try {
                var linkHref = new URL($(this).attr('href'), window.location.href).href;
                if (linkHref === currentUrl) {
                    $(this).addClass("on");
                    return false; // 找到匹配后退出.each()循环
                }
            } catch (error) {
                console.error("Invalid href attribute: ", $(this).attr('href'));
            }
        });
    }
